...
export default function DragElement({ footerRef }: DragElementProps) {
  ...
  return (
    <div
      className={`grid gap-2 place-items-center bg-[#9e9e9e] bg-opacity-20 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 hide-scrollbar ${data.length == 0? "" : "p-[1rem]"}`}
    >
      {imageList}
    </div>
  );
