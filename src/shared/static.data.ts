...
export const staticData: IstaticData[] = [
  ...
  },
  {
    ...
    details: {
      ...
      params: '최초 구상은 Next를 이용해 간단한 페이지를 만드는 것이였으나, 개발 과정에서 추가되는 기능들과 배포 환경에서의 문제점 때문에 확장되는 기능을 관리하기 위해 각 기능을 모듈별로 세분화 하고 검증했습니다. \n • 요청사항이 n개 이상 쌓였을 때 메일 알림이 오는 기능\n • 데이터베이스에 기록되어있는 일자를 기준으로 중복 메일이 전송되지 않게 검증하는 기능\n •  각 이미지 요소를 메모제이션 하는 기능 들이 들어가 있습니다.',
      about: [
        {
          placeholder: 'notion',
  ...
  },
  {
    ...
    details: {
      ...
      params: '안타깝게도 해당 해커톤은 본선 진출이 무산되었지만 평소 잘 모르던 도메인에 대한 자료 조사와 팀간 의견 조율을 진행했던 과정입니다.',
      about: [
        {
          placeholder: 'notion',
